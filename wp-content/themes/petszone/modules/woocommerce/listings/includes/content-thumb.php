...
if( ! function_exists( 'petszone_shop_woo_loop_product_thumbnail' ) ) {

	function petszone_shop_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_style != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.petszone_html_output($product_overlay_bgcolor).'></div>';
				}
				...
				if($product_show_offer_percentage) {
					echo petszone_shop_woo_loop_product_offer_percentage($product);
				}


				// Action to run before product thumb image
				do_action( 'petszone_woo_before_product_thumb_image', $product_id );
				...
				if($display_bg_image) {
					...
					echo '<div class="primary-image"><img src="'.esc_url($primary_image_src).'" alt="'.esc_attr__('Shop Primary Image', 'petszone').'" title="'.esc_attr__('Shop Primary Image', 'petszone').'" /></div>';
				}
				...
				if($secondary_image_src != '') {
					if($display_bg_image) {
						...
						echo '<div class="secondary-image"><img src="'.esc_url($secondary_image_src).'" alt="'.esc_attr__('Shop Secondary Image', 'petszone').'" title="'.esc_attr__('Shop Secondary Image', 'petszone').'" /></div>';
					}
				}
			...
			if($product_thumb_content) {
				...
				petszone_shop_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.petszone_html_output($product_thumb_content_overlay_bgcolor).'>';
					do_action('petszone_woo_loop_product_thumb_content', 'thumb');
					remove_all_actions('petszone_woo_loop_product_thumb_content');
				echo '</div>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'petszone_shop_woo_loop_product_thumbnail', 10 );
