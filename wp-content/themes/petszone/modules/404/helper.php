<?php
add_action( 'petszone_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'petszone-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, PETSZONE_THEME_VERSION, 'all');
    }
}

add_filter( 'petszone_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = petszone_404_page_params();
...
function petszone_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'petszone_404_page_params', $params );
}