			...
			if( is_array($settings['hotspots']) && !empty($settings['hotspots']) ) {
				$output .= '<div class="wdt-hotspot-items-holder">';
					foreach ( $settings['hotspots'] as $key => $hotspot ) 
					{
						...
						$tooltip_content = '';
						//icon additional starts
						if($settings['hotspot_type'] == 'icon') {
							if($hotspot['media_image1']['url'])
							{
								$media_image_item = array ();
								$media_image_item['image'] = isset($settings['media_image1']) ? $settings['media_image1'] : '';
								// $media_image_item['image_size'] = $settings['media_image_size'];
								// $media_image_item['image_custom_dimension'] = isset($settings['media_image_custom_dimension']) ? $settings['media_image_custom_dimension'] : array ();
								$tooltip_content .= '<div class="wdt-hotspot-item-icon-content">';
									$tooltip_content .= $this->cc_repeater_contents->render_image($hotspot, '', '');
								$tooltip_content .= '</div>';		
							}

						}
						//icon additional ends
						if($hotspot['show_tooltip_content'] && $hotspot['tooltip_content']) {
							$tooltip_content.='<div class="wdt-hotspot-item-content">';
