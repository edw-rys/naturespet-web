...
class WeDesignTech_Common_Controls_Repeater_Contents {
	...
	public function get_controls($elementor_object) {
		...
		$elementor_object->start_controls_section( 'wdt_section_content', array(
			...
			if(in_array('default', $this->options_group)) {
				...
				if(in_array('addtional_icon', $this->option_default_keys)) {
					$repeater->add_control(
						'media_icon1',
						array (
							'label' => esc_html__( 'Additional Icon', 'wdt-elementor-addon' ),
							'type' => \Elementor\Controls_Manager::ICONS,
							'default' => (isset($this->module_details['icon_default_library']) && !empty($this->module_details['icon_default_library'])) ? $this->module_details['icon_default_library'] : array( 'value' => 'fas fa-star', 'library' => 'fa-solid' ),
							'condition' => array_merge(
								$this->option_default_condition,
	...
	public function render_title($item_title, $link_start, $link_end) {
		...
		if( !empty( $item_title ) ) {
			$output .= '<div class="wdt-content-title">';
				$output .= '<h5>';
					$output .= $link_start;
					$output .= wp_kses_post( $item_title );
					$output .= $link_end;
				$output .= '</h5>';
			$output .= '</div>';
		}
	...
	public function render_image($item, $link_start = '', $link_end = '') {
		...
		$renderImage = function ($media_image) use ($link_start, $link_end, $item) {
			$output = '';
			if (!empty($media_image['url'])) {
				...
				$media_image_setting = [
					'image' => $media_image,
					'image_size' => 'full',
					'image_custom_dimension' => isset($item['media_image_custom_dimension']) ? $item['media_image_custom_dimension'] : [],
				];
		...
		if (isset($item['media_image']) && !empty($item['media_image'])) {
			$output .= $renderImage($item['media_image']);
		}

		if (isset($item['media_image1']) && !empty($item['media_image1'])) {
			$output .= $renderImage($item['media_image1']);
		}
	...
	public function render_icon($key, $item, $widget_object) {
		...
		if ( ! empty( $item['media_icon1']['value'] ) ) :

			$output .= '<div class="wdt-content-icon-wrapper-1">';
				$output .= '<div class="wdt-content-icon"><span>';
					$output .= ($item['media_icon1']['library'] === 'svg') ? '<i>' : '';
						ob_start();
						\Elementor\Icons_Manager::render_icon( $item['media_icon1'], [ 'aria-hidden' => 'true' ] );
						$output .= ob_get_clean();
					$output .= ($item['media_icon1']['library'] === 'svg') ? '</i>' : '';
				$output .= '</span></div>';
			$output .= '</div>';
