...
if( ! function_exists( 'petszone_shop_woo_additional_product_tabs' ) ) {

	function petszone_shop_woo_additional_product_tabs( $tabs ) {
		...
		if( is_array( $product_additional_tabs ) && !empty( $product_additional_tabs ) ) {
			...
			foreach( $product_additional_tabs as $product_additional_tab_key => $product_additional_tab ) {
				...
				$tab_key = 'petszone_'.strtolower($tab_key);

				$tabs[$tab_key] = array(
					'title' 	=> $tab_title,
					'priority' 	=> $priority,
					'callback' 	=> 'petszone_shop_woo_additional_product_tabs_content'
				);
	...
	function petszone_shop_woo_additional_product_tabs_content( $key ) {
	...
	add_filter( 'woocommerce_product_tabs', 'petszone_shop_woo_additional_product_tabs', 10 );
