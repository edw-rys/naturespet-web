...
if( !class_exists( 'PetsZoneProSidebar' ) ) {
    class PetsZoneProSidebar {
        ...
        function __construct() {
            ...
            $this->global_layout  = petszone_customizer_settings('global_sidebar_layout');
            $this->global_sidebar = petszone_customizer_settings('global_sidebar');
            $this->hide_standard_sidebar = petszone_customizer_settings('hide_standard_sidebar');
        ...
        function load_modules() {
            include_once PETSZONE_PRO_DIR_PATH.'modules/sidebar/customizer/index.php';
            include_once PETSZONE_PRO_DIR_PATH.'modules/sidebar/metabox/index.php';
        }

        function frontend() {
            add_action('petszone_after_main_css', array( $this, 'enqueue_assets' ) );
            add_filter('petszone_primary_classes', array( $this, 'primary_classes' ), 20 );
            add_filter('petszone_secondary_classes', array( $this, 'secondary_classes' ), 20 );
            add_filter('petszone_active_sidebars', array( $this, 'active_sidebars' ), 20 );
        }

        function enqueue_assets() {
            wp_enqueue_style( 'site-sidebar', PETSZONE_PRO_DIR_URL . 'modules/sidebar/assets/css/sidebar.css', false, PETSZONE_PRO_VERSION, 'all' );
            $id       = get_the_ID();
            $settings = get_post_meta( $id, '_petszone_layout_settings', true );
            $settings = is_array( $settings ) ? array_filter( $settings ) : array();

            if( ( isset($settings['sticky_sidebar'] ) && !empty($settings['sticky_sidebar'] ) ) ) {
                wp_enqueue_style( 'sidebar', PETSZONE_PRO_DIR_URL . 'modules/sidebar/assets/css/sidebar.css', false, PETSZONE_PRO_VERSION, 'all' );
                wp_enqueue_script( 'theia-sticky-sidebar', PETSZONE_PRO_DIR_URL . 'assets/js/theia-sticky-sidebar.min.js', array('jquery'), PETSZONE_PRO_VERSION, true );
                wp_enqueue_script( 'sidebar-sticky', PETSZONE_PRO_DIR_URL . 'modules/sidebar/assets/js/side-bar.js', array('theia-sticky-sidebar'), PETSZONE_PRO_VERSION, true );
            }
        }
        ...
        function primary_classes( $primary_class ) {
            ...
            if( is_singular( $this->sidebar_post_types )  ) {
                $settings = get_post_meta( get_queried_object_id(), '_petszone_layout_settings', true );
                $settings = is_array( $settings ) ? array_filter( $settings ) : array();
                ...
                if( isset( $settings['layout'] ) ) {
                    if( $settings['layout'] == 'global-sidebar-layout' ) {
                        ...
                        if($this->hide_standard_sidebar) {
                            unset($sidebars[array_search('petszone-standard-sidebar-1', $sidebars)]);
                        }
                    } else {
