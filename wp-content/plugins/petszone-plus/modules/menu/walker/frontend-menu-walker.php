<?php
if ( ! class_exists( 'PetsZonePlusFrontendMenuWalker' ) ) {

	class PetsZonePlusFrontendMenuWalker {
		...
		function enqueue_css_assets() {
			wp_enqueue_style( 'dtplugin-nav-menu-animations', PETSZONE_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu-animations.css', false, PETSZONE_PLUS_VERSION, 'all');
			wp_enqueue_style( 'dtplugin-nav-menu', PETSZONE_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu.css', false, PETSZONE_PLUS_VERSION, 'all');
		}

		function enqueue_js_assets() {
			wp_enqueue_script( 'dtplugin-mega-menu', PETSZONE_PLUS_DIR_URL . 'modules/menu/assets/js/mega-menu.js', array(), PETSZONE_PLUS_VERSION, true );
		}
	}
}

PetsZonePlusFrontendMenuWalker::instance();

if( ! class_exists( 'PetsZone_Walker_Nav_Menu' ) ) {

	class PetsZone_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( is_object( $args ) && property_exists( $args, 'before' ) ) {
				$item_output  = $args->before;
			} else {
				$item_output = '';
			}
			$item_output .= '<a' . petszone_html_output($attributes) . '>';
				...
				if( !empty( $item->icon ) || !empty( $item->image ) ) {
					...
					if( !empty( $item->image ) ) {
						$item_output .= '<i class="menu-item-image">';
							$item_output .= '<img src="'.esc_url( $item->image ).'" alt="'.esc_attr( 'Image', 'petszone-plus' ).'"/>';
						$item_output .= '</i>';
					}
				}
